...
export default function Canvas2014() {
    ...
    let hei = ctx.canvas.height
        ...
        ctx.beginPath();
        ctx.moveTo((wid/2)-10,hei/3)
        ctx.lineTo((wid/2),35)
        ctx.lineTo((wid/2)+10,hei/3)
        ctx.lineTo((wid/2)-10,hei/3)
        ctx.fill()
        ...
        ctx.beginPath()
        ctx.arc(51,8,8,0*Math.PI,2*Math.PI)
        ctx.fill()

      };
      return <Canvas draw={fleche} className="formcanvas" id="canvas2014" />;
}
